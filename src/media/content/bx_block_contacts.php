<?php
/* -----------------------------------------------------------------------------------------
   $Id: /media/content/bx_block_contacts.php 14390 2023-05-22 07:28:14Z benax $
    _                           
...
// BX Block email addresses START
if (defined('MODULE_BLOCK_CONTACTS_STATUS') && MODULE_BLOCK_CONTACTS_STATUS === 'true') {
  if (defined('MODULE_BLOCKED_CONTACTS') && !empty(MODULE_BLOCKED_CONTACTS)) {
    $blocked_senders = json_decode(MODULE_BLOCKED_CONTACTS, true);
    $parts = explode('@', trim($email));
    if  (in_array($email, $blocked_senders['emails']) || 
         in_array($parts[1], $blocked_senders['domains']) || 
         in_array($parts[0], $blocked_senders['localparts'])) {
      $messageStack->add('contact_us', ERROR_EMAIL);
      $error = true;
    }
  }
}
// BX Block email addresses END
    ...
    if (in_array('contact', $use_captcha) && (!isset($_SESSION['customer_id']) || MODULE_CAPTCHA_LOGGED_IN == 'True')) {    
      if (isset($_POST['vvcode']) && $mod_captcha->validate($_POST['vvcode']) !== true) {
